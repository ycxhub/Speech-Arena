-- process_vote: Model-level ELO (Phase 2)
-- Resolves models.id to (provider_id, model_id), updates elo_ratings_global_model and elo_ratings_by_language_model
...
BEGIN
  ...
  FROM public.models WHERE id = p_winner_id;
  IF v_winner_provider_id IS NULL OR v_winner_model_id IS NULL THEN
  ...
  FROM public.models WHERE id = p_loser_id;
  IF v_loser_provider_id IS NULL OR v_loser_model_id IS NULL THEN
    RAISE EXCEPTION 'Loser model not found: %', p_loser_id;
  END IF;

  -- Enforce: winner and loser must be different models (not same model with different voices)
  IF v_winner_provider_id = v_loser_provider_id AND v_winner_model_id = v_loser_model_id THEN
    RAISE EXCEPTION 'Winner and loser cannot be the same model (provider_id=%, model_id=%)', v_winner_provider_id, v_winner_model_id;
  END IF;

  -- Ensure elo_ratings_global_model rows exist (lazy init)
  INSERT INTO public.elo_ratings_global_model (provider_id, model_id, rating, matches_played, wins, losses)
  VALUES (v_winner_provider_id, v_winner_model_id, 1500, 0, 0, 0)
  ON CONFLICT (provider_id, model_id) DO NOTHING;
  INSERT INTO public.elo_ratings_global_model (provider_id, model_id, rating, matches_played, wins, losses)
  VALUES (v_loser_provider_id, v_loser_model_id, 1500, 0, 0, 0)
  ON CONFLICT (provider_id, model_id) DO NOTHING;
  ...
  WHERE provider_id = v_winner_provider_id AND model_id = v_winner_model_id
  FOR UPDATE;

  SELECT rating, matches_played, wins, losses INTO v_loser_rating, v_loser_matches, v_loser_wins, v_loser_losses
  FROM public.elo_ratings_global_model
  WHERE provider_id = v_loser_provider_id AND model_id = v_loser_model_id
  FOR UPDATE;
  ...
  LEFT JOIN public.elo_ratings_by_language_model er
    ON er.provider_id = v_winner_provider_id AND er.model_id = v_winner_model_id AND er.language_id = p_language_id;

  SELECT COALESCE(er.rating, 1500), COALESCE(er.matches_played, 0), COALESCE(er.wins, 0), COALESCE(er.losses, 0)
  INTO v_loser_lang_rating, v_loser_lang_matches, v_loser_lang_wins, v_loser_lang_losses
  FROM (SELECT 1) dummy
  LEFT JOIN public.elo_ratings_by_language_model er
    ON er.provider_id = v_loser_provider_id AND er.model_id = v_loser_model_id AND er.language_id = p_language_id;
  ...
  INSERT INTO public.elo_ratings_by_language_model (provider_id, model_id, language_id, rating, matches_played, wins, losses)
  VALUES (v_winner_provider_id, v_winner_model_id, p_language_id, v_winner_new_lang, v_winner_lang_matches + 1, v_winner_lang_wins + 1, v_winner_lang_losses)
  ON CONFLICT (provider_id, model_id, language_id) DO UPDATE SET
    rating = EXCLUDED.rating,
    matches_played = elo_ratings_by_language_model.matches_played + 1,
    wins = elo_ratings_by_language_model.wins + 1,
    last_updated = now();
  INSERT INTO public.elo_ratings_by_language_model (provider_id, model_id, language_id, rating, matches_played, wins, losses)
  VALUES (v_loser_provider_id, v_loser_model_id, p_language_id, v_loser_new_lang, v_loser_lang_matches + 1, v_loser_lang_wins, v_loser_lang_losses + 1)
  ON CONFLICT (provider_id, model_id, language_id) DO UPDATE SET
    rating = EXCLUDED.rating,
    matches_played = elo_ratings_by_language_model.matches_played + 1,
  ...
  -- Update test_events with vote results and ELO snapshots (model-level ratings)
